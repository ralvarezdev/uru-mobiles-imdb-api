...
HOST_ARCH=$(uname -m)
KDB_ARCH_TAH="latest" # Default for x86/amd64 systems on KurrentDB Docker image
...
# Check for 32-bit ARM (Older Raspberry Pi or 32-bit OS)
  ...
  echo "Host architecture detected as 32-bit ARM. Falling back to default tag: $KDB_ARCH_TAH (Check if a 32-bit tag is needed)."

# Default is x86/amd64
else
  echo "Host architecture detected as x86/amd64. Using default tag: $KDB_ARCH_TAH"
fi
